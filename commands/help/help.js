...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if(args[0]) {
            ...
            .addField("Fun:", "``8ball`` ``advice`` ``anime`` ``asciify`` ``clyde`` ``decrypt`` ``didumean`` ``emoji`` ``encrypt`` ``facts`` ``flip`` ``fml`` ``gayrate`` ``howdumb`` ``kill`` ``kiss`` ``meme`` ``morse`` ``oof`` ``pickup`` ``programming`` ``rate`` ``roll`` ``slap`` ``trump`` ``yomama`` ")
