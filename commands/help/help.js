...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if(args[0]) {
            ...
            .addField("Fun", "``8ball`` ``advice`` ``asciify`` ``clyde`` ``decrypt`` ``didumean`` ``emoji`` ``encrypt`` ``facts`` ``flip`` ``fml`` ``gayrate`` ``howdumb`` ``kill`` ``meme`` ``morse`` ``oof`` ``programming`` ``rate`` ``roll`` ``slap`` ``trump`` ``yomama``")
            ...
            .addField("NSFW (Only Premium Users)", "``ass`` ``pussy`` ``tits`` ``4k`` ``random``")
