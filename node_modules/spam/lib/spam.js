...
// is this version 6.0 or later
var recentVersion = compareVersions(process.versions.node, '6.0.0') >= 0;
...
function handleExit(child, worker, code, signal) {
	...
	var disconnect = recentVersion ? worker.exitedAfterDisconnect : worker.suicide;

	if (disconnect) {
		status.event = 'disconnect';
		spam.emit('status', status);
		logMessage('               worker disconnected (fka suicide)');
	} else {
		spam.emit('status', status);
		if (signal) {
